...
```{python}
pre_order_label_dict = {0: 3, 1: 1, 2: 2, 3: 0}
```
...
for ardt_name in tqdm(ARDT_NAMES):
    ...
    if ardt_name == "Mundhenk_v3":
        labels = remap_labels(label_ds.labels.data, pre_order_label_dict)
...
label_dict["Mundhenk_v3"] = pre_order_label_dict
```
...
def annual_average_plot(ar_ds, pr_ds, suptitle, ann_avg_precip_ds):
    ...
    freq_data = ar_ds.ar_tracked_id * 100
    vmax = np.ceil(freq_data.max().values)
    freq_data.plot(
        ax=axs[0],
        vmin=0,
        vmax=vmax,
    ...
    pr_frac = pr_ds / ann_avg_precip_ds * 100
    vmax = np.ceil(pr_frac.max().values)
    pr_frac.plot(
        ax=axs[2],
        vmin=0,
        vmax=vmax,
    ...
    for i, data in enumerate([freq_data, pr_ds, pr_frac]):
...
Get the time series ranges.

```{python}
ar_ts_trend.median("ardt").quantile([0.025, 0.975])
```

```{python}
ar_ts_trend.max("ardt").quantile([0.025, 0.975])
```

```{python}
ar_ts_trend.min("ardt").quantile([0.025, 0.975])
```
...
ax.format(
    suptitle="Total annual AR frequency and average annual precipitation over Scandinavia",
    title="",
    xlabel="Year",
    ylabel="AR frequency [%]",
)
...
Manually adjust the bins

```{python}
bins_avg = np.linspace(0, 1, 11)
bins_avg_open = bins_avg.copy()
bins_avg_open[-1] = 2
```
...
```{python}
bins_max = np.linspace(0, 30, n_bins + 1)
bins_max_open = bins_max.copy()
bins_max_open[-1] = 80
bins_max_open
```
...
ax[0].format(
    xlocator=bins_avg[::2],
    xlim=[0, 1],
    xticklabels=["0", "0.2", "0.4", "0.6", "0.8", "$\geq$1"],
)
ax[0].format(
    xminorlocator=bins_avg[1::2],
)
ax[1].format(
    # xminorlocator=3,
    xlocator=bins_max[::2],
    xlim=[0, 30],
    xticklabels=["0", "6", "12", "18", "24", "$\geq$30"],
)

ax[1].format(
    xminorlocator=bins_max[1::2],
)
...
def ar_cluster_plot(ar_freqs, ar_pr, suptitle):
    ...
    cmap = plt.Colormap("oslo_r", right=0.8)
    for i in range(n_clusters):
        freq_data = ar_freqs.isel(label=i).ar_tracked_id * 100
        cm = freq_data.plot.pcolormesh(
            ax=axs[i, 0],
            vmin=0,
            rasterized=True,
            add_colorbar=False,
        ...
        curr_tp = ar_pr.isel(label=i).tp / ann_avg_precip_ds * 100
        cm = curr_tp.plot.pcolormesh(
            ax=axs[i, 1],
            vmin=0,
            cmap=cmap,
            rasterized=True,
        ...
        axs[i, 1].colorbar(
            cm, label="Total precipitation\nfraction [%]", width=0.15, extend="max"
        )
        ...
        for j, data in enumerate([freq_data, curr_tp]):
            local_max = data.max().values
            axs[i, j].annotate(
                f"Max: {local_max:.1f}",
                (1, 0),
    ...
    axs.format(
        ...
        latlim=LAT_SLICE,
        title="",
        rowlabels=[f"AR pathway {i}" for i in range(1, n_clusters + 1)],
...
```{python}
n_clusters = 4
fig, ax = plt.subplots(nrows=4, figwidth="8.5cm", figheight="12cm")
win_size = 10
# Isn't this just spatial mean?
for i in range(4):
    for j in range(4):
        (data.isel(label=i, ardt=j).mean("year")).plot(
            ax=ax[i], label=data.ardt.values[j], lw=1
        )
...
def season_cluster_plot(freqs, suptitle):
    ...
    for i in range(nrows):
        ...
        for j, season in enumerate(season_order):
            data = freqs.isel(label=i).sel(season=season).ar_tracked_id
            cm = data.plot(
                ax=axs[i, j],
                rasterized=True,
                vmin=0,
...
nao_bins = IntervalIndex(
    [
        Interval(-3.501, -1.501),
        Interval(-1.501, -0.501),
        Interval(-0.501, 0.5),
        Interval(0.5, 1.5),
        Interval(1.5, 3.5),
    ]
...
# hist, bins = nao_series.groupby(nao_bins)
```
...
precip_ds_nao = precip_ds.assign_coords({"nao": nao_da})
```
...
ar_nao_groups_counts = precip_ds_nao.time.groupby(nao=BinGrouper(nao_bins)).count()
```

```{python}
ar_nao_groups_counts = ar_nao_groups_counts.compute()
```

```{python}
# Reshape for broadcasting
nao_counts = ar_nao_groups_counts.values.reshape((1, 1, -1, 1, 1))
```

Calculate the frequencies

```{python}
freqs = ar_nao_groups / ar_nao_groups_counts * 100
```
...
def nao_cluster_plot(freqs, suptitle):
    ...
    bin_labels = [
        "NAO$\\mathbf{<-1.5}$",
        "$\\mathbf{-1.5\\leq}$NAO$\\mathbf{<-0.5}$",
        "$\\mathbf{-0.5\\leq}$NAO$\\mathbf{\\leq 0.5}$",
        "$\\mathbf{0.5 <}$NAO$\\mathbf{\\leq 1.5}$",
        "$\\mathbf{1.5<}$NAO",
    ]
    ...
    axs.format(
        ...
        title="",
        rowlabels=[f"AR pathway {i}" for i in range(1, n_clusters + 1)],
        collabels=[f"{bin}" for bin in bin_labels],
        toplabelpad=0,
        suptitle=suptitle,
    )
    return fig
...
### NAO cluster with geop anomalies

```{python}
daily_mean_ds = xr.open_zarr(
    "/data/era5/geopotential/geopotential-daiy_mean-1979_2020.zarr/"
).sel(valid_time=slice("1980", "2019"))
```

```{python}
daily_mean_ds = daily_mean_ds.assign_coords(
    {"nao": ("valid_time", nao_series.resample("1D").mean())}
)
```

```{python}
nao_geop_neutral = (
    daily_mean_ds.groupby(nao=BinGrouper(nao_bins))
    .mean()
    .isel(pressure_level=0, nao_bins=2)
    .compute()
)
```

```{python}
nao_geop_neutral = nao_geop_neutral.drop_vars("nao_bins")
```

```{python}
nao_geop_anom = (
    (daily_mean_ds - nao_geop_neutral)
    .groupby(nao=BinGrouper(nao_bins))
    .mean()
    .isel(pressure_level=0)
)
```

```{python}
nao_geop_anom = nao_geop_anom.compute()
```

```{python}
nao_geop_anom_sel = nao_geop_anom.sel(
    latitude=slice(90, 10), longitude=slice(-130, 155)
)
```

```{python}
def nao_cluster_anom_plot(freqs, nao_geop, nao_geop_neutral, suptitle):
    n_clusters = freqs.label.shape[0]
    nrows = n_clusters + 1
    ncols = 5
    fig, axs = plt.subplots(
    ...
    for i in range(5):
        if i == 2:
            data = nao_geop_neutral.z / 98.2
            vmin = 520  # data.min().values
            vmax = 590  # data.max().values
            cmap = plt.Colormap("batlow", left=0.2)
            levels = 11
            norm = None
        else:
            ...
            vmin = -25
            vmax = 25
            #levels = np.asarray([-27, -22, -17, -12, -7, -2, 2, 7, 12, 17, 22, 27])
            levels = [-25, -20, -15, -10, -5, 5, 10, 15, 20, 25]
            cmap = plt.Colormap("vik")
            norm = plt.DivergingNorm(vcenter=0, vmin=vmin, vmax=vmax)

        cm_tmp = (data).plot.contourf(
            ax=axs[0, i],
            levels=levels,
            lw=0.4,
            vmin=vmin,
    ...
    #     width=0.1,
    #     #ticks=[-22, -11, 0, 11, 22],
    #     label="Geopotential height anomaly [dm]",
    #     rasterized=True,
    # )
    ...
    axs.format(
        rowlabels=["z500"] + [f"AR pathway {i}" for i in range(1, n_clusters + 1)],
        collabels=[f"{bin}" for bin in bin_labels],
        toplabelpad=0,
        suptitle=suptitle,
    )
...
```{python}
fig = nao_cluster_anom_plot(
    freqs.median("ardt"),
    nao_geop_anom_sel,
    nao_geop_neutral,
    "NAO-grouped AR frequencies [Ensemble median]",
)
```

```{python}
fig.savefig("../../figures/nao_geop_ar_clusters_ens_med.svg")
```
...
fig = nao_cluster_anom_plot(
    ...
    nao_geop_anom_sel,
    nao_geop_neutral,
    "NAO-grouped AR frequencies [Ensemble min]",
)
```

```{python}
fig.savefig("../../figures/nao_geop_ar_clusters_ens_min.svg")
```
...
fig = nao_cluster_anom_plot(
    ...
    nao_geop_anom_sel,
    nao_geop_neutral,
    "NAO-grouped AR frequencies [Ensemble max]",
)
```

```{python}
fig.savefig("../../figures/nao_geop_ar_clusters_ens_max.svg")
```
...
for cluster in range(4):
    for nao in range(5):
        ...
        res_inner = np.zeros(5)
        for nao_2 in range(5):
            data_cand = (
...
def nao_dist_plot(freqs: xr.Dataset, title: str):
    ...
    for cluster in range(4):
        for nao in range(5):
            ...
            for nao_2 in range(5):
                ...
                res_inner[nao_2] = scstats.wasserstein_distance(data_cand, data_curr)
            res[cluster, nao] = res_inner.mean()

    fig, ax = plt.subplots(figwidth="8.5cm", sharey=False, hratios=(0.7, 0.3), nrows=2)
    ...
    for i in range(4):
        for j in range(5):
            ...
            x = (2.5 * j) + (0.5 * i)
            parts = ax[0].violin(
                x, data.values.flatten(), fc=f"C{i}", label=f"AR pathway {i+1}"
            )

    for i in range(4):
        ax[1].plot(np.arange(0.75, 11.5, 2.5), res[i], label=f"AR pathway {i+1}")

    handles, labels = ax[0].get_legend_handles_labels()

    fig.legend(handles[::5], labels[::5], ncols=2, loc="b")

    ax[0].format(ylabel="AR frequency [%]")
    ax[1].format(ylabel="Mean\nWasserstein distance")
    ...
    ax.format(
        xticks=np.arange(0.75, 11.5, 2.5),
        xticklabels=[f"{bin}" for bin in bin_labels],
        xrotation=20,
