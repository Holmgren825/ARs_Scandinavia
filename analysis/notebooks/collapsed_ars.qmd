...
for i in range(4):
    ardt = ar_trend_max.isel(ardt=i).ardt.values
    corr = sc_stats.pearsonr(
        ann_precip.values, ann_ar_precip_max.isel(ardt=i).values
    )
    res.append([ardt, corr.statistic, corr.pvalue])
```
...
ar_ts_trend.min("ardt").plot(ax=ax, label="ARDT min-max", c="k", lw=1, ls="--", zorder=5, alpha=0.6)
...
for i in range(4):
    ardt = ar_ts_trend.isel(ardt=i)
    corr = sc_stats.pearsonr(
        ann_ar_precip.isel(ardt=i).values, ardt.values
    )
    res.append([ardt.ardt.values, corr.statistic, corr.pvalue])
```
...
```{python}
n_ts_per_month = precip_ds.valid_time.groupby("valid_time.month").count()
```

```{python}
weights = np.cos(np.deg2rad(ar_ens_ds.lat))
group_ar_cycle = (
    (ar_ens_ds.groupby(["start_time.month", "label", "ardt"]).sum())
    / n_ts_per_month
).compute()
```
...
```{python}
data = (
    (group_ar_cycle).weighted(weights).quantile(0.75, dim=["lat", "lon"])
)
```
...
for i in range(4):
    (data.isel(label=i).median("ardt").ar_tracked_id).plot(
        ax=ax[i], label="Ensemble Median", lw=1
    )
    cycle_max = data.isel(label=i).max("ardt").ar_tracked_id
    cycle_min = data.isel(label=i).min("ardt").ar_tracked_id
    ax[i].fill_between(
...
for cluster in range(4):
    for nao in range(4):
        data_curr = freqs.isel(label=cluster, nao_bins=nao).ar_tracked_id.median("ardt").values.flatten()
        res_inner = np.zeros(4)
        for nao_2 in range(4):
            data_cand = freqs.isel(label=cluster, nao_bins=nao_2).ar_tracked_id.median("ardt").values.flatten()
...
def nao_dist_plot(freqs: xr.Dataset, title: str):
    ...
    for cluster in range(4):
        for nao in range(4):
            data_curr = freqs.isel(label=cluster, nao_bins=nao).ar_tracked_id.values.flatten()
            res_inner = np.zeros(4)
            for nao_2 in range(4):
                data_cand = freqs.isel(label=cluster, nao_bins=nao_2).ar_tracked_id.values.flatten()
                res_inner[nao_2] = scstats.wasserstein_distance(data_cand, data_curr)
            res[cluster, nao] = res_inner.sum()
            
    fig, ax = plt.subplots(figwidth="8.5cm", abc=True, sharey=False, hratios=(0.7, 0.3), nrows=2)
    ...
    for i in range(4):
        for j in range(4):
            ...
            data = data.fillna(0)
            x = (2.5*j)+(0.5*i)
            parts = ax[0].violin(x, data.values.flatten(), fc=f"C{i}", label=f"Cluster {i+1}")
...
```{python}
fig = nao_dist_plot(freqs.median("ardt"), title="NAO-grouped AR frequency distributions [Ensemble median]")
```
...
```{python}
fig = nao_dist_plot(freqs.min("ardt"), title="NAO-grouped AR frequency distributions [Ensemble min]")
```
...
```{python}
fig = nao_dist_plot(freqs.max("ardt"), title="NAO-grouped AR frequency distributions [Ensemble max]")
```
