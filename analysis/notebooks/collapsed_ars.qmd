...
(ar_ann_avg_std_frq.ar_tracked_id * 100).plot(
    ax=axs[0], vmin=0, cbar_kwargs={"label": "AR frequency [%]"}, rasterized=True
)
# vmax = max(n_ar_ann_avg.max().values, ar_ann_avg.max().values)
...
```{python}
fig, ax = pplt.subplots(figwidth="12cm", sharey=True, ncols=2, abc=True)
...
```{python}
avg_res = np.zeros((4, 4))
ext_res = np.zeros((4, 4))
for label in range(4):
    for ardt in range(4):
...
ax.format(
    xlabel="Day of year",
    ylabel="NAO",
    suptitle="Annual average NAO [1980-2019]"
)
```
