...
```{python}
ar_precip_ens = precip_ds.cf.sel(time=ar_pr_ts_ens.valid_time)
n_ar_precip_ens = precip_ds.where(
    ~precip_ds.cf["time"].isin(ar_pr_ts_ens.valid_time), drop=True
)
```
...
```{python}
n_ar_ann_avg = (
    n_ar_precip_ens.cf.groupby("time.year").sum().mean("year").tp.compute() / 4
)
...
(ar_ann_avg / (ar_ann_avg + n_ar_ann_avg) * 100).plot(
    ax=axs[1],
...
```{python}
test = ar_precip_ens.assign_coords({"label": ar_ts_ens})
```

```{python}
test2 = test.groupby(["label", "valid_time.year"]).sum().mean("year").load()
```
...
vmax = test2.tp.max().values
for i in range(4):
    test2.isel(label=i).tp.plot(ax=axs[i], vmin=0, cmap="oslo_r", rasterized=True)
...
axs.format(
    ...
    suptitle="Annual average precipitation\nper cluster [Ens. average]"
)
```

```{python}
fig.savefig("../../figures/ens_avg_ar_pr_clusters.svg")
```
