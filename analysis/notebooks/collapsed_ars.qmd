...
```{python}
ar_ann_avg_frq = (
    (ar_ens_ds.groupby(["ardt", "start_time.year"]).sum() / n_days.values)
    .mean("year")
    .median("ardt")
    .load()
)
...
(ar_ann_avg_frq.ar_tracked_id * 100).plot(
...
```{python}
fig.savefig("../../figures/ann_avg_precip_ar_ens_med.svg")
```
...
for i in range(4):
    (test.isel(ardt=i).ar_tracked_id / 1460 * 100).plot(
...
```{python}
corr = compute_spatial_correltion(
    ar_ann_avg_max_frq.ar_tracked_id, (ar_ann_max_pr / ann_avg_precip_ds * 100)
)
```
...
(ar_ann_avg_max_frq.ar_tracked_id * 100).plot(
    ax=axs[0], vmin=0, cbar_kwargs={"label": "AR frequency [%]"}, rasterized=True
)
# vmax = max(n_ar_ann_avg.max().values, ar_ann_avg.max().values)
cm = ar_ann_max_pr.plot(
    ax=axs[1], vmin=0, cmap="oslor_r", add_colorbar=False, rasterized=True
)
# cm = ar_ann_avg.plot(ax=axs[1], vmin=0, vmax=vmax, cmap="oslor_r", add_colorbar=False)
axs[1].colorbar(cm, label="Total annual average\nprecipitation [mm]")
...
cmap = plt.Colormap("oslo_r", right=0.8)
(ar_ann_max_pr / ann_avg_precip_ds * 100).plot(
    ax=axs[2],
    vmin=0,
    cmap=cmap,
    cbar_kwargs={"label": "Total precipitation\nfraction [%]"},
...
```{python}
fig.savefig("../../figures/ann_avg_precip_ar_ens_max.svg")
```
...
```{python}
corr = compute_spatial_correltion(
    ar_ann_avg_min_frq.ar_tracked_id, (ar_ann_min_pr / ann_avg_precip_ds * 100)
)
```
...
(ar_ann_avg_min_frq.ar_tracked_id * 100).plot(
    ax=axs[0], vmin=0, cbar_kwargs={"label": "AR frequency [%]"}, rasterized=True
)
# vmax = max(n_ar_ann_avg.max().values, ar_ann_avg.max().values)
cm = ar_ann_min_pr.plot(
    ax=axs[1], vmin=0, cmap="oslor_r", add_colorbar=False, rasterized=True
)
# cm = ar_ann_avg.plot(ax=axs[1], vmin=0, vmax=vmax, cmap="oslor_r", add_colorbar=False)
axs[1].colorbar(cm, label="Total annual average\nprecipitation [mm]")
...
cmap = plt.Colormap("oslo_r", right=0.8)
(ar_ann_min_pr / ann_avg_precip_ds * 100).plot(
    ax=axs[2],
    vmin=0,
    cmap=cmap,
    cbar_kwargs={"label": "Total precipitation\nfraction [%]"},
...
```{python}
fig.savefig("../../figures/ann_avg_precip_ar_ens_min.svg")
```
...
for ardt_data in ar_trend.values.T:
    ...
    mk_res_avg.append(results.p)
mk_res_ext = []
for ardt_data in ar_trend_max.values.T:
    results = mk.original_test(ardt_data)
    mk_res_ext.append(results.p)
```

```{python}
weights = np.cos(np.deg2rad(precip_ds.latitude))
ann_precip = precip_ds.groupby("valid_time.year").sum().weighted(weights).mean(["latitude", "longitude"]).tp.compute()
ann_precip_max = precip_ds.groupby("valid_time.year").sum().max(["latitude", "longitude"]).tp.compute()
```

```{python}
grid_spec = [[1, 2], [3, 3]]
fig, ax = plt.subplots(grid_spec, hratios=[1, 0.8], figwidth="12cm", sharey=True, spany=False, abc=True)

time = ar_trend.year

# Spatial averages
for i in range(4):
    label = f"{ar_trend.isel(ardt=i).ardt.values}\np avg: {mk_res_avg[i]:.2f}\np ext:{mk_res_ext[i]:.2f}"
    (ar_trend.isel(ardt=i) * 100).plot(ax=ax[0, 0], label=label)

# Spatial extremes
for i in range(4):
    (ar_trend_max.isel(ardt=i) * 100).plot(ax=ax[0, 1])

#ann_precip.plot(ax=ax[1, 0], c="C5")
ax[1, 0].bar(ann_precip.year, ann_precip_max.values, label="Spatial max", cycle="colorblind10", c="C11")
ax[1, 0].bar(ann_precip.year, ann_precip.values, label="Spatial avg.", cycle="colorblind10", c="C0")
...
ax[0, :].format(ylabel="Ann. AR frequency [%]")
ax[1, :].format(ylabel="Total annual precipitation [m]")
ax.format(
    xlabel="Year",
    #ylabel=["Ann. AR frequency [%]", "Test"],
    suptitle="Annual AR frequency and precipitation",
...
```{python}
# Average. Divide by 6 to get average hourly.
ar_avg_pr_ts = ar_precip_ens_nz.groupby("ardt").mean().median("ardt").load() / 6
...
cmap = plt.Colormap("oslo_r", right=0.8)
for i in range(n_clusters):
    cm = (grouped_ars.isel(label=i).ar_tracked_id * 100).plot(
        ax=axs[i, 0],
        vmin=0,
        rasterized=True,
        add_colorbar=False,
...
axs.format(
    ...
    suptitle="AR clusters [Ensemble med.]",
)
```
...
cmap = pplt.Colormap("oslo_r", right=0.8)
for i in range(n_clusters):
    cm = (freq_cluster_std.isel(label=i).ar_tracked_id * 100).plot(
        ax=axs[i, 0],
        vmin=0,
        rasterized=True,
        add_colorbar=False,
...
```{python}
fig = season_cluster_plot(freqs, "Seasonal AR frequencies [Ensemble med.]")
```

```{python}
fig.savefig("../../figures/ens_med_seasonal_ar_clusters.svg")
```
...
season_groups_max = (
    (
        ar_ens_ds.groupby(
            ["ardt", "label", "start_time.season", "start_time.year"]
        ).sum()
        / n_days.values.reshape((1, 1, 1, -1))
    )
    .mean("year")
...
```{python}
freqs = season_groups_max * 100
```

```{python}
fig = season_cluster_plot(freqs, "Seasonal AR frequencies [Ensemble max]")
```

```{python}
fig.savefig("../../figures/ens_max_seasonal_ar_clusters.svg")
```

```{python}
freqs = season_groups_min * 100
```

```{python}
fig = season_cluster_plot(freqs, "Seasonal AR frequencies [Ensemble min]")
```

```{python}
fig.savefig("../../figures/ens_min_seasonal_ar_clusters.svg")
```
...
```{python}
fig = nao_cluster_plot(freqs, "NAO-grouped AR frequencies [Ensemble med.]")
```
...
```{python}
freqs = ar_nao_max_groups / nao_counts * 100
```

```{python}
fig = nao_cluster_plot(freqs, "NAO-grouped AR frequencies [Ensemble max]")
```

```{python}
fig.savefig("../../figures/ens_max_nao_ar_clusters.svg")
```

```{python}
freqs = ar_nao_min_groups / nao_counts * 100
```

```{python}
fig = nao_cluster_plot(freqs, "NAO-grouped AR frequencies [Ensemble min.]")
```

```{python}
fig.savefig("../../figures/ens_min_nao_ar_clusters.svg")
```
