...
```{python}
# Convert to mm/h
ar_precip_ens = ar_precip_ens * 1e3 / 6
ar_precip_ens_unique = ar_precip_ens_unique * 1e3 / 6
n_ar_precip_ens = n_ar_precip_ens * 1e3 / 6
```

```{python}
weights = np.cos(np.deg2rad(ar_precip_ens_unique.latitude))

# Mean
ar_precip_hist = ar_precip_ens_unique.weighted(weights).mean(["latitude", "longitude"])
n_ar_precip_hist = n_ar_precip_ens.weighted(weights).mean(["latitude", "longitude"])

# Max (0.75 quantile)
ar_precip_hist_max = ar_precip_ens_unique.quantile(0.75, ["latitude", "longitude"])
n_ar_precip_hist_max = n_ar_precip_ens.quantile(0.75, ["latitude", "longitude"])
```
...
```{python}
# mean
ar_med = ar_precip_ens.cf.mean("valid_time").compute()
n_ar_med = n_ar_precip_ens.cf.mean("valid_time").compute()
# std
...
pr_cbar = axs[1].colorbar(
    pr_cm, loc="bottom", label="Precipitation [mm/h]", extend="max"
)
...
```{python}
# mean
ar_std = ar_precip_ens.cf.std("valid_time").compute()
```
...
vmax = max(ar_med.tp.values.max(), ar_std.tp.values.max())
pr_cm = (ar_med.tp).plot(
    ax=axs[0], cmap=cmap, vmin=0, vmax=vmax, add_colorbar=False, rasterized=True
)
...
pr_cbar = axs[1].colorbar(
    pr_cm, loc="bottom", label="Precipitation [mm/h]"
)
...
axs.format(
    ...
    suptitle="Ensemble average NAO-grouped AR frequencies",
)
```
