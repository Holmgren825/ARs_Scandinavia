...
def annual_average_plot(ar_ds, pr_ds, suptitle, ann_avg_precip_ds):
    ...
    (ar_ds.ar_tracked_id * 100).plot(
        ax=axs[0], vmin=0, cbar_kwargs={"label": "AR frequency [%]"}, rasterized=True, cmap="bamako"
    )
    # vmax = max(n_ar_ann_avg.max().values, ar_ann_avg.max().values)
    cmap = plt.Colormap("oslo_r", right=0.8)
    cm = pr_ds.plot(
        ax=axs[1], vmin=0, cmap=cmap, add_colorbar=False, rasterized=True
    )
    ...
    axs.format(
        ...
        latlim=LAT_SLICE,
        title="",
        rowlabels=["AR frequency", "Total AR precipitation", "AR precipitation fraction"],
        suptitle=suptitle,
        abcloc="ul",
        abcbbox=True,
...
for i in range(4):
    ...
    corr = sc_stats.pearsonr(ar_trend.isel(ardt=i).values, ann_tp_max.values)
...
for i in range(4):
    ...
    corr = sc_stats.pearsonr(ar_trend_max.isel(ardt=i).values, ann_tp_max.values)
    res.append([ardt, corr.statistic, corr.pvalue])
...
for i in range(4):
    ...
    corr = sc_stats.pearsonr(
        ar_trend_max.isel(ardt=i).values, ann_precip_max.isel(ardt=i).values
    )
    res.append([ardt, corr.statistic, corr.pvalue])
```
...
for i in range(4):
    ...
    annotation = f"{ar_trend.isel(ardt=i).ardt.values}, p-val$_{{avg}}$: {mk_res_avg[i]:.2f}, p-val$_{{P_{{99}}}}$: {mk_res_ext[i]:.2f}"
    # ax[i].annotate(annotation, (0, 0))
    ...
    bar_ax[i].bar(
        ann_precip.year,
        ann_precip_max.isel(ardt=i).values,
...
fig.legend(handles, labels, loc="b", ncols=3)
# bar_ax.format(ylabel="AR Total Precipitation [m]")
fig.text(
    0.985,
    0.55,
    "AR Total Precipitation [m]",
    va="center",
    ha="center",
    rotation="vertical",
    in_layout=True,
...
#    alpha=0.2,
# )
ens_min = np.min(hist_ar, axis=0)
ens_min = np.concat([ens_min[:1], ens_min])

ens_max = np.max(qtile_hist_ar, axis=0)
...
ax.fill_between(
    bins,
    ens_min,
    np.where(ens_max > ens_med_pad, ens_max, ens_med_pad),
    color="C0",
    step="pre",
    alpha=0.2,
)

ax.stairs(ens_med, bins, label="AR [Avg.]", color="C0")
# ax.stairs(
#    np.median(qtile_hist_ar, axis=0), bins, label="AR [3rd qtile]", color="C0", ls="--"
# )
#
## Non AR conditions.
ax.stairs(hist_n_ar, bins, label="No AR [Avg.]", color="C1")
...
This will be the average number of AR pixels? Or just absolute.

```{python}
n_samples_per_day = precip_ds.valid_time.groupby("valid_time.dayofyear").count()
```

```{python}
test = ar_ens_ds.groupby(["start_time.dayofyear", "label", "ardt"]).sum().compute()
```

```{python}
test = test.ar_tracked_id / n_samples_per_day
```
...
group_ar_cycle = (
    (
        ar_ens_ds.groupby(["start_time.dayofyear", "label", "ardt"]).sum()
        / n_samples_per_day
    )
    # .weighted(weights)
    # .mean(["lat", "lon"])
...
```{python}
n_gridpoints = ar_ens_ds.lat.shape[0] * ar_ens_ds.lon.shape[0]
```
...
dates = pd.to_datetime(data.dayofyear.values, format="%j")
data = data.assign_coords({"dayofyear": dates}) * 100  # / n_gridpoints * 100
```
...
for i in range(4):
    (
        data.isel(label=i)
        .rolling(dayofyear=win_size, min_periods=1, center=True)
        .mean()
        .median("ardt")
        .ar_tracked_id
    ).plot(ax=ax[i], label="Ensemble Med.", lw=1)
    (
        data.isel(label=i)
        .rolling(dayofyear=60, min_periods=1, center=True)
        .mean()
        .median("ardt")
        .ar_tracked_id
    ).plot(ax=ax[i], label="60-day window", lw=1, ls="--")
    ...
    ax[i].fill_between(
        cycle_min.dayofyear, cycle_min, cycle_max, alpha=0.5, label="Ensemble Min - Max"
    )
...
ax.format(
    title="",
    xlabel="Month",
    ylabel="Spatial P$_{75}$ AR probability [%]",
    rowlabels=[f"Cluster {i}" for i in range(1, n_clusters + 1)],
    suptitle="Average AR seasonal cycle",
    xlocator=MonthLocator(bymonth=range(1, 13)),
    xtickminor=False,
    xformatter="%b",
    xrotation=0,
    ymin=0,
...
```{python}
n_days_season = precip_ds.valid_time.groupby(["valid_time.year", "valid_time.season"]).count()
```
...
```{python}
season_grouped_ars = season_grouped_ars / n_days_season.T.values.reshape(1, 1, n_seasons, n_years, 1, 1)
...
fig = nao_cluster_plot(
    freqs.median("ardt"), "NAO-grouped AR frequencies [Ensemble med.]"
)
```
