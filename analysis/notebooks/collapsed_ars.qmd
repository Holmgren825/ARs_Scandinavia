...
def annual_average_plot(ar_ds, pr_ds, suptitle, ann_avg_precip_ds):
    ...
    cm = pr_ds.plot(
        ax=axs[1], vmin=0, vmax=vmax, levels=11, cmap=cmap, add_colorbar=False, rasterized=True, robust=False
    )
...
ar_nao_groups_counts = precip_ds_nao.time.groupby(
    nao=BinGrouper(nao_bins)
).count()
```
