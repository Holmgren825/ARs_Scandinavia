...
for i in range(5):
    ...
    ax[i].format(
        title=f"{bin_labels[i]}, n: {counts.z.isel(nao_bins=i).values:.0f}",
    )
...
```{python}
nao_geop_anom = (
    (daily_mean_ds - daily_mean_ds.mean("valid_time"))
...
vmin = np.round(nao_geop_anom.z.min().values / 98.2)
vmax = np.round(nao_geop_anom.z.max().values / 98.2)
print(vmin, vmax)
vmin = -26
vmax = 26
...
fig.colorbar(cm, span=(2, 4), loc="b", width=0.1, label="Geopotential height [dm]", rasterized=True)
...
ax.format(
    ...
    latlim=(30, 90),
    lonlim=(-70, 70),
    suptitle="Daily average geopotential height anomaly at 500 hPa grouped by NAO-phase",
    # rowlabels=["Negative", "Neutral", "Positive"],
)
```

```{python}
fig.savefig("anomalies_text.svg")
```
