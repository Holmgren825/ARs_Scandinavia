...
```{python}
ds = xr.open_mfdataset(path_zarr, engine="zarr", chunks="auto")
ds = ds.thin({"time": 6})
```

```{python}
ds.ar_binary_tag.sel(time=slice("1979", "1981"))
```
...
```{python}
gdf = gpd.read_file("../etc/ne_110m_admin_0_countries/ne_110m_admin_0_countries.shp")
```
...
```{python}
scand_gdf.loc[scand_gdf.index == 21, "geometry"] = (
    scand_gdf.loc[scand_gdf.index == 21, "geometry"].iloc[0].geoms[1]
)
```
...
### Select the IDs that intersect the mask.
...
season_freqs.to_netcdf("/data/projects/atmo_rivers_scandinavia/ERA5_6hr_Guan_Waliser_season_freqs.nc")
```
...
nao_freqs.to_netcdf("/data/projects/atmo_rivers_scandinavia/ERA5_6hr_Bin_Waliser_nao_freqs.nc")
```
