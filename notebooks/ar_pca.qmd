...
```{python}
fnames_single = {
    "comp_path": "ar_eofs_components.zarr",
    "exp_var_path": "ar_eofs_exp_var.zarr",
    "scores_path": "ar_eofs_scores.zarr",
}
```
...
```{python}
pca_model = ComputePCA(
    data=[sel_ds.ar_features, sel_resample_ds.tp],
    model=model,
    base_path=PROJ_BASE_PATH,
    result_fnames=fnames_6h,
    combined_pca=True,
...
```{python}
fnames = {
    "comp_name": "ar_pr_resample_eofs_components.zarr",
    "exp_var_name": "ar_pr_resample_eofs_exp_var.zarr",
    "scores_name": "ar_pr_resample_eofs_scores.zarr",
}
```
...
```{python}
comp, exp_var, scores = load_pca_results(PROJ_BASE_PATH, fnames)
```
